...
function setup() {
  ...
  fatalitySlider = createSlider(0, 100, 40);
  ...
  vaccineSlider = createSlider(0, 1, 0);
  ...
  externalTravelSlider = createSlider(0, 100, 40);
...
function lockSliders() {
  ...
  externalTravelSlider.value(40);
  if (!hospitalOverrun) {
    fatalitySlider.value(40);
  } else {
    fatalitySlider.value(80);
  }
...
function timeIt() {
  if (isLooping && started) {
    ...
    if (currentCompliance > 0) {
      complianceSlider.value(complianceSlider.value() - 2);
      currentCompliance -= 2;
    }
