...
function setup() {
  ...
  periodSlider = createSlider(1, 10, 4);
  ...
  externalTravelSlider = createSlider(0, 100, 99);
  ...
  setInterval(lowerCompliance, 3000);
...
function lockSliders() {
  ...
  periodSlider.value(4);
  externalTravelSlider.value(99);
  if (!hospitalOverrun) {
    fatalitySlider.value(30);
  } else {
    fatalitySlider.value(60);
  }
  if (accumulatedCost > 125000000) {
...
function createPopulation() {
  let infCom = communities[1];
  let carrier = new Person3(personSize, "asymptomatic", infCom);
  for (let i = 0; i <= 2; i++) {
