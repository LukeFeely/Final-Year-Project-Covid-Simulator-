...
function setup() {
  ...
  capacitySlider = createSlider(5, 20, 10);
  ...
  vaccineSlider = createSlider(0, 3, 0);
...
// function getRandomInfectedPerson() {
//   let randomPerson = population[getRandomInt(0, population.length - 1)];
//   if (randomPerson.currentCommunity == communities[4]) {
//     return randomPerson;
//   } else {
//     return getRandomInfectedPerson();
//   }
// }
...
function updateText() {
  ...
  select("#travelText").html(
    `Travel Restrictions: ${travelSlider.value() - 20}%   `
  );
...
function checkSuccess() {
  if ((dead.length / population.length) * 100 <= 10) {
    select("#success").html(`Completed`);
    select("#congrats").html(
      `You succeeded in your goal of keeping deaths at 10% or less, with the €100,000,000 budget you were allocated.`
    );
  } else {
    select("#failed").html(`Failed`);
    select("#congrats").html(
      `You failed in your goal of keeping deaths under 10% with the €100,000,000 budget you were allocated.`
    );
  }
...
function createCommunities() {
  ...
  let com2 = new Community2(20, height * 0.33, width / 2, height / 3 - 50);
